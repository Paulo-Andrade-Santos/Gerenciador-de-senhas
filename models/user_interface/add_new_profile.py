...
class NewProfile:
    def __init__(self):
        ...
        self.__main_window.configure(background="#DAA520")
    ...
    def __make_labels(self):
        ...
        Label(master=self.__main_window, text="Senha:", font=("Courier New", 15, "bold"), background="#DAA520").place(x=self.__X_axis, y=130)
    ...
    def __make_entrys(self):
        ...
        self.__username.configure(font=("Courier New", 15, "bold"), width=30)
        self.__username.place(x=self.__X_axis, y=80)
        self.__pass_entry: Entry = Entry(master=self.__main_window)
        self.__pass_entry.configure(font=("Courier New", 15, "bold"), width=30, show="•")
        self.__pass_entry.place(x=self.__X_axis, y=160)

    def __make_button(self):
        """Método responsável por desenhar os botões (buttons)."""
        Button(master=self.__main_window, text="Registrar", font=("Courier New", 13, "bold"), cursor="hand1", command=self.__insert_record).place(x=self.__X_axis, y=250)
        Button(master=self.__main_window, text="Voltar", font=("Courier New", 13, "bold"), width=10, cursor="hand1", command=self.__welcome_screen).place(x=self.__X_axis + 255, y=250)

    def __make_check_buttons(self):
        state: IntVar = IntVar()
        Checkbutton(master=self.__main_window, activebackground="#DAA520", text="Mostar senha", font=("Courier New", 10, "bold"), background="#DAA520", variable=state, command=lambda: self.__pass_entry.configure(show="") if state.get() == 1 else self.__pass_entry.configure(show="•")).place(x=self.__X_axis - 5, y=190)
    ...
    def __insert_record(self):
        if len(self.__username.get()) != 0 and len(self.__pass_entry.get()) != 0:
            DataBase().insert_records(self.__username.get(), self.__pass_entry.get())
